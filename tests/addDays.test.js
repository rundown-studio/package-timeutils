...
describe('addDays', () => {
  it('should handle DST transition in Australia/Sydney (Spring Forward)', () => {
    ...
    expect(result).to.deep.equal(new Date('2023-10-01T13:30:00.000Z')) // Mon Oct 02 2023 00:30:00 GMT+1100 (Australian Eastern Daylight Time)
  })
  ...
  it('should work correctly with UTC', () => {
    ...
    const result = addDays(date, 1, { timezone })
    expect(result).to.deep.equal(new Date('2023-05-16T12:00:00.000Z'))
  })
  ...
  it('should work correctly with Europe/Berlin', () => {
    ...
    const result = addDays(date, 1, { timezone })
    expect(result).to.deep.equal(new Date('2023-03-26T22:00:00.000Z')) // Mar 27 2023 00:00:00 GMT+0200 (CEST) - after DST change
  })
  ...
  it('should work correctly with America/Los_Angeles', () => {
    ...
    const result = addDays(date, 1, { timezone })
    expect(result).to.deep.equal(new Date('2023-11-06T08:00:00.000Z')) // Mon Nov 06 2023 01:00:00 GMT-0800 (Pacific Standard Time)
  })
  ...
  it('should handle negative day additions', () => {
    ...
    const result = addDays(date, -1, { timezone })
    expect(result).to.deep.equal(new Date('2023-03-25T23:00:00.000Z')) // Mar 26 2023 00:00:00 GMT+0100 (CET) - before DST change
  })
  ...
  it('should handle large day additions America/Los_Angeles', () => {
    ...
    expect(result).to.deep.equal(new Date('2024-01-01T05:00:00.000Z')) // Jan 01 2024 00:00:00 GMT-0500 (EST)
  })

  it('should throw an error for invalid date input', () => {
    expect(() => addDays('not a date', 1)).to.throw('`date` must be an instance of Date')
  })

  it('should throw an error for invalid amount input', () => {
    expect(() => addDays(new Date(), 'not a number')).to.throw('`amount` must be a number')
  })
})
